...
# (apps/projector/project-rules.sh) from the one portable source.
...
# tools are chosen. The git hooks + the quality scripts they call are the
# "enforcement travels with the repo" guarantee, so every install gets them.
REFRESH_PATHS_COMMON="
.githooks
apps/quality
"

# Claude-only template-owned paths (the index generators the Claude skills call).
# Refreshed only when Claude Code is among the selected tools.
REFRESH_PATHS_CLAUDE="
apps/README.md
apps/app-index
apps/project-index
...
install_tool_projection() {
  bash "$SCRIPT_DIR/apps/projector/project-rules.sh" "$1" "$TARGET" \
...
if tool_selected claude "$SELECTED_TOOLS"; then
  ...
  for rel in $REFRESH_PATHS_CLAUDE; do
    refresh_path "$rel"
  done
fi
...
# Tool-agnostic enforcement (git hooks + the quality scripts they call) — always.
