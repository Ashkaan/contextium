...
|---|---|---|
| Think | `/project` | Render the project index, or start/route a project and write a lean SPEC. |
| Do | `/implement` | Execute a SPEC with self-validation. Starts fresh on purpose. |
| Wrap | `/close` | Journal the session, then commit. |

Supporting skills: `/probe` (adversarial self-review), `/explain` (deep investigation), `/debate`
(argue a decision from both sides), `/propose-rule` (add a rule the right way).
