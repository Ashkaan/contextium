...
#   worktree remove -f   → pass iff the target working tree is clean (--force
#                          then discards nothing); else block.
#
...
# worktree remove --force is safe when the target's working tree is clean —
# --force then discards nothing. Any uncommitted change → block.
worktree_remove_is_safe() {
  local path porcelain seg
...
if echo "$COMMAND" | grep -qE 'git[[:space:]]+worktree[[:space:]]+remove\b[^|;&]*(--force\b|[[:space:]]-f\b)'; then
  # State-aware: pass when the target worktree is clean. Fail-closed on any
  # uncommitted change.
