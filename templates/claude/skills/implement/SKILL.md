...
steps:
  ...
  - id: auto-close
    ...
    gate:
      tool: skill
      on_fail: halt
      condition: "Terminal auto-close gate. Code built + validated, mechanism-match clean, implement-audit clean, no depth-policy decision / deferral outstanding → auto-invoke /close per .claude/skills/close/references/auto-close-gate.md (check close-fired.sh status, mark, dispatch /close to journal + commit + push). A mechanism-match FAIL or an outstanding decision HALTS here — auto-close does not fire."
---
